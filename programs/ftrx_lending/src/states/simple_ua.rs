...
pub struct SimpleUa{
    ...
    pub liquid_stable_minted:u64,
    pub liquid_volatile_minted:u64,
    pub stable_asset_share_locked:u64,
    pub volatile_asset_share_locked:u64
...
impl SimpleUa{
    ...
    pub fn get_max_new_borrowable_volatile_share(
        ...
        -> Option<(u64)>{
            ...
            let free_stable_share=self.user_stable_share_deposited.checked_sub(self.stable_asset_share_locked).unwrap();
            msg!["LOG USDC amount  collateral {}/1000000 vs free collateral {}/1000000",self.user_stable_share_deposited,free_stable_share];
            
            let usdc_amount=free_stable_share.checked_mul(simple_pool.stable_share_asset_value).unwrap().checked_div(SHARE_VALUE_ADAPTER).unwrap();
            msg!["LOG USDC amount free collateral {}/1000000",usdc_amount];
    ...
    pub fn get_max_new_mint_liquid_stable(
        &mut self,
        simple_pool:&SimplePool,
        price_pool:i64,
        price_multiplier:u64,
        ...
        -> Option<(u64)>{
            ...
            let max_liquid_stable_value_new_borrow=usdc_amount.checked_mul(simple_pool.new_borrow_max_ltv).unwrap().checked_div(BASE_MULTIPLIER).unwrap();

            //Amount with 6 decimals, needs to be adapted later
            Some(max_liquid_stable_value_new_borrow)
    }
    ...
    pub fn get_max_maintainance_borrowable_volatile(
        ...
        -> Option<(u64)>{
            ...
            msg!["LOG USDC amount free collateral {}/1000000 {}",usdc_amount,self.user_stable_share_deposited];
    ...
    pub fn get_max_new_borrowable_stable(
        ...
        -> Option<(u64)>{
            ...
            let free_volatile_share=self.user_volatile_share_deposited.checked_sub(self.volatile_asset_share_locked).unwrap();
            msg!["LOGuser volatile_share deposited {}/1000000 vs free {}/1000000",self.user_volatile_share_deposited,free_volatile_share];
            let volatile_amount=free_volatile_share.checked_mul(simple_pool.volatile_share_asset_value).unwrap().checked_div(SHARE_VALUE_ADAPTER).unwrap();
    ...
    pub fn get_max_new_mint_liquid_volatile(
        &mut self,
        simple_pool:&SimplePool,
        price_pool:i64,
        price_multiplier:u64,
        ...
        -> Option<(u64)>{
            ...
            let volatile_amount=free_volatile_share.checked_mul(simple_pool.volatile_share_asset_value).unwrap().checked_div(SHARE_VALUE_ADAPTER).unwrap();
            msg!["LOG Volatile amount collateral {}/1000000",volatile_amount];
            let max_volatile_amount_liquid_mint=volatile_amount.checked_mul(simple_pool.new_borrow_max_ltv).unwrap().checked_div(BASE_MULTIPLIER).unwrap();;

            Some(max_volatile_amount_liquid_mint)
    }
